...
class preprocesses:
    ...
    def collect_data(self):
        ...
        # Add a datetime key to the filename to allow alignment using multiple processes
        # random_key = np.random.randint(0, high=99999)
        timeNow = datetime.now()
        datetimeStr = timeNow.strftime("%d%m%Y_%H%M%S")
        bounding_boxes_filename = os.path.join(output_dir, 'bounding_boxes_%s.txt' % datetimeStr)
